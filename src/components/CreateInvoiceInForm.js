...
import InvoiceInFormComponent from './InvoiceInFormComponent';
...
class CreateInvoiceInForm extends Component {
    ...
    deleteDoc = (id, e) => {
        e.stopPropagation()
        var grFilter = {}
        this.props.delData('invoiceins', grFilter, id)
    ...
    render() {
        ...
        const check = (
            <Menu>
                <Menu.Item key="0">
                    <Link to={{ pathname: '/checkPayment', search: `${this.props.id}`, hash: "invoiceins" }} onClick={this.handlePageCheck} target={'_blank'} >A4</Link>
                ...
                <Menu.Item key="2">
                    <Link to={{ pathname: '/checkPayment', search: `${this.props.id}`, hash: "invoiceins" }} onClick={this.handlePageCheck} target={'_blank'} >80mm </Link>
        ...
        return (
            <div className='table_holder'>
                ...
                <MobileDrawer
                    docbuttons={
                        <DocButtons errorFields={errorFields} from={this.props.fromdoc ? this.props.fromdoc : 'p=transactions'} linkedDocs={this.props.state.datas.doc ? true : false} doc={this.props.state.datas.doc} toDoc='' fromDoc='createInvoiceIn' buttonsName={buttonsNames} activeitem={this.props.state.navbar.activeItem} activesubitem={this.props.state.navbar.activeSubItem} />
                ...
                <Row className='filter_table_wrapper_row doc'>
                    <Col className='page_name_wrapper' xs={24} md={24} xl={24}>
                        <h2 className='custom_top_margin'><Trans word={'InvoiceIn'} /></h2>
                    </Col>
                    ...
                    <InvoiceInFormComponent docid={this.props.id ? this.props.state.datas.doc[0].Id : ''} saledoc={this.props.saledoc} datas={this.props.state.spenditems.spendItems} doc={this.props.id ? this.props.state.datas.doc[0] : ''} />
...
export default connect(mapStateToProps, mapDispatchToProps)(LoaderDocHOC(CreateInvoiceInForm, 'fetching'))