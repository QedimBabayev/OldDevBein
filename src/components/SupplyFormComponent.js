...
class SupplyFormComponent extends Component {
  ...
  onFinish = (values) => {
    ...
    if (description === "") {
      sendObject.description = "";
    }
    sendObject.consumption = consumption
      ? consumption
      : this.props.doc.Consumption;
    if (consumption === "") {
      sendObject.consumption = "";
    }
    ...
    sendObject.status = this.state.status;
    if (!this.state.sendRequest) {
      progress(true);
      this.props.saveDocument(
        "supplies",
        sendObject,
        this.props.state.savedoc.toPayment
      );
    }
    ...
    var totalAmount = 0;
    sendObject.positions.forEach(
      (c) => (totalAmount += parseFloat(c.TotalPrice))
    );
    this.props.updateDoc(sendObject, totalAmount);
  };
  ...
  render() {
    ...
    return (
      <>
        ...
        <PaymentFrom
          handleOk={this.openPay}
          handleCancel={this.closePay}
          title={"Məxaric"}
          message={"Məxaric sənədi saxlanıldı"}
          visible={this.state.showPayment}
          to={"paymentouts"}
          doc={this.props.state.datas.doc[0]}
        />
            ...
            <Row className="top_wrapper_holder">
              <Col xs={24} md={24} xl={6}>
                <div className="first_form_wrapper">
                  <Form.Item
                    label={<Trans word={"Supply Number"} />}
                    name="name"
                  ...
                  <div className="form_top_side doc_permission">
                    <Collapse ghost>
                      <Panel header="Təyinat" key="1">
                        <Row>
                          <Col className="permissioncol" xs={24} md={24} xl={6}>
                            <Form.Item
                              label={"Cavabdeh"}
                              name="ownerid"
                              style={{ margin: "0" }}
...
export default connect(
  mapStateToProps,
  mapDispatchToProps
)(LoaderHOC(SupplyFormComponent, "datas"));
