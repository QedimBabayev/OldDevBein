...
class PaymentInFormComponent extends Component {
  ...
  onChangeSpendItem = (value, option) => {
    ...
    if (option.staticname === "correct") {
      ...
      console.log("fields", this.formRef.current.getFieldsValue().customerid);

      if (
        this.formRef.current.getFieldsValue().customerid ===
  ...
  if (
    ...
    if (!this.state.sendRequest) {
      progress(true);
      this.props.saveDocument("paymentins", sendObject);
    }
    this.setState({
      sendRequest: true,
    });
  ...
  render() {
    ...
    const spendOptions = Object.values(this.props.state.spenditems.spendItems)
      .filter(
        (item) =>
          item.StaticName === "buyproduct" || item.StaticName === "correct"
      )
      .map((item) => (
        <Option
          key={item.Id}
...
export default connect(
  mapStateToProps,
  mapDispatchToProps
)(LoaderHOC(PaymentInFormComponent, "datas"));
